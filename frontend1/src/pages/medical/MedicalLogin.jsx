...
const MedicalLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${API_URL}/medical/login`, formData);
      
      if (response.data.success) {
        localStorage.setItem('medical_token', response.data.token);
        localStorage.setItem('medical_data', JSON.stringify(response.data.staff));
        ...
        <Card className="border-white/20 bg-white/10 backdrop-blur-md shadow-2xl">
          <CardHeader className="space-y-2 pb-6">
            ...
            <p className="text-center text-slate-200 text-sm font-light">
              Sign in to manage student health records
            </p>
          </CardHeader>
